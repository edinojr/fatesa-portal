...
const CertificateVerification = () => {
  ...
  return (
    <PublicPageLayout title="Verificação de Certificado" subtitle="Valide a autenticidade de certificados emitidos pela Fatesa.">
      <section className="public-section" style={{ paddingTop: '3rem' }}>
        <div style={{ maxWidth: '640px', margin: '0 auto', textAlign: 'center' }}>
          <div style={{ marginBottom: '2rem' }}>
            <Logo size={200} />
          </div>

          <div className="auth-card" style={{ maxWidth: '100%', padding: '2.5rem', textAlign: 'left' }}>
            <p style={{ textAlign: 'center', color: 'var(--text-muted)', marginBottom: '2rem' }}>
              Insira o código impresso no certificado para validar os dados do formado.
            </p>

            <form onSubmit={(e) => { e.preventDefault(); handleVerify(code); }} style={{ display: 'flex', gap: '0.75rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
            ...
            {loading && (
              <div style={{ textAlign: 'center', padding: '2rem' }}>
                ...
                <p style={{ marginTop: '1rem', color: 'var(--text-muted)' }}>Consultando registros oficiais...</p>
              </div>
            )}
            ...
            {result && (
              <div style={{ 
                ...
                <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
                  ...
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Registro vinculado à Fatesa - Casa do Saber</p>
                </div>

                <div style={{ display: 'grid', gap: '1rem', fontSize: '1rem' }}>
                  <div style={{ borderBottom: '1px solid var(--glass-border)', paddingBottom: '0.75rem' }}>
                    <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem', display: 'block', textTransform: 'uppercase' }}>Nome do Formado</span>
                    <strong style={{ fontSize: '1.2rem' }}>{result.nome}</strong>
                  </div>
                  <div style={{ borderBottom: '1px solid var(--glass-border)', paddingBottom: '0.75rem' }}>
                    <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem', display: 'block', textTransform: 'uppercase' }}>Curso / Especialidade</span>
                    <strong>{result.curso}</strong>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                    <div>
                      <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem', display: 'block', textTransform: 'uppercase' }}>Nível</span>
                      <strong>{result.nivel__curso || 'N/A'}</strong>
                    </div>
                    <div>
                      <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem', display: 'block', textTransform: 'uppercase' }}>Ano de Formação</span>
                      <strong>{result.ano_formacao}</strong>
                    </div>
                  </div>
                  <div>
                    <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem', display: 'block', textTransform: 'uppercase' }}>Polo / Núcleo</span>
